...
class OnboardingSerializer(serializers.ModelSerializer):
    ...
    append_mode     = serializers.BooleanField(default=False, required=False)
    ...
    def _apply_preferences(self, user, account_type, preferences_data, append_mode):
        """Shared logic used by both create() and update()."""
        user.account_type = account_type

        if append_mode:
            existing_leagues = set(
                FanPreference.objects.filter(user=user).values_list('league_id', flat=True)
            )
            ...
            # Full replace (first-time onboarding or reset)
            ...
            if account_type == 'fan' and preferences_data:
                ...
                user.fan_badge = (
                    f"{primary['team'].name} Fan" if primary.get('team') else None
                )

        if account_type == 'news':
            user.fan_badge = (
                'Official Media' if user.badge_type == 'official' else 'Awaiting Partnership'
            )
    ...
    def create(self, validated_data):
        ...
        return self._apply_preferences(user, account_type, preferences_data, append_mode)

    def update(self, instance, validated_data):
        """Delegate to the same logic as create(), operating on the given instance."""
        account_type     = validated_data.get('account_type', instance.account_type)
        preferences_data = validated_data.get('fan_preferences', [])
        append_mode      = validated_data.get('append_mode', False)
        return self._apply_preferences(instance, account_type, preferences_data, append_mode)
...
class CustomLoginSerializer(LoginSerializer):
    ...
    dj-rest-auth's default enforces email — we override that here.
    """
    ...
    def validate(self, attrs):
        ...
        # Email-based lookup: resolve to username first, then authenticate
        if '@' in identifier:
            try:
